...
namespace CustomProxyBrowser
{
    ...
    {
        SHDocVw.WebBrowser activeBrowser;
        ...
        {
            Form1 newWindow = new Form1(bstrUrl);
            newWindow.Show();
            Cancel = true;
        }  

        void activeBrowser_StatusTextChange(string Text)
        {
            this.Invoke(new Action(delegate
            {
        ...
        private void AdjustToolbar()
        {
            try
            {
                    ...
                    {
                        if (!item.Name.Equals("toolStripTextBox1"))
                        {
                            if (item.Visible) totalWidth += item.Width;
                        }
                    }
                    if (totalWidth > 0)
                    {

                        if ((this.Width - totalWidth - 100) > 0)
                        {
                            toolStripTextBox1.Width = this.Width - totalWidth - 80;
                        }
                        else
                        {
                            toolStripTextBox1.Width = 10;
                        }
                    }
        ...
        {
            activeBrowser.ExecWB(SHDocVw.OLECMDID.OLECMDID_FIND, SHDocVw.OLECMDEXECOPT.OLECMDEXECOPT_PROMPTUSER);
        }
